  ...
  <NavLink
    ...
    className={({ isActive }) =>
      `flex items-center rounded-lg py-2.5 font-medium transition-all ${
        collapsed
          ? "justify-center px-0 w-10 shrink-0 mx-auto"
          : "gap-3 px-3 w-full"
      } ${
        isActive
    ...
    <>
      ...
      <aside
        className={`flex h-screen shrink-0 flex-col border-r border-slate-800 bg-slate-950 transition-all duration-300 z-50 ${
          collapsed
            ? "w-0 md:w-[72px] -translate-x-full md:translate-x-0"
            : "w-64 translate-x-0 fixed md:relative"
        }`}
      >
        <div
          className={`flex h-16 items-center border-b border-slate-800 ${collapsed ? "justify-center" : "justify-between px-5"}`}
        >
          <div
            className={`flex items-center gap-2.5 overflow-hidden ${collapsed ? "w-0 h-0 opacity-0" : "opacity-100"}`}
          >
        ...
        <nav className="flex-1 space-y-8 overflow-y-auto overflow-x-hidden p-3 custom-scrollbar">
          ...
          {recentProjects.length > 0 && (
            <div className="flex flex-col gap-1">
              {collapsed ? (
                <div
                  className="mx-auto mt-4 mb-2 flex items-center justify-center text-slate-500"
                  title="Recent"
                >
        ...
        <div
          className={`flex border-t border-slate-800 p-3 ${collapsed ? "justify-center" : ""}`}
        >
          {collapsed ? (
            <span className="text-[10px] text-slate-600 font-medium">v1.0</span>
          ) : (
            <p className="px-3 text-xs text-slate-600 font-medium">
              TaskFlow v1.0.0
            </p>
          )}
        </div>
