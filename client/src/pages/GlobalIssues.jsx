...
import { Search, Loader, Filter, CheckCircle, Clock, ArchiveX, XCircle, Layout, ArrowRight } from "lucide-react";

const GlobalIssues = () => {
  const navigate = useNavigate();
  const [filter, setFilter] = useState({ q: "", projectId: "all", priority: "all", status: "all" });
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    return task.title.toLowerCase().includes(lowerQ) || task.content?.toLowerCase().includes(lowerQ);
  });

  const getPriorityColor = (prio) => {
    switch (prio) {
      case "critical": return "text-red-400 font-bold bg-red-400/10 border-red-500/20";
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "completed":
        return <span className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-emerald-400"><CheckCircle className="size-3.5" /> Done</span>;
      case "active":
        return <span className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-indigo-400"><Clock className="size-3.5" /> Active</span>;
      case "cancelled":
        return <span className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400"><ArchiveX className="size-3.5" /> Cancelled</span>;
      case "rejected":
        return <span className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-red-400"><XCircle className="size-3.5" /> Rejected</span>;
      default:
        return <span className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">{status}</span>;
    }
  };
  ...
  return (
    <div className="flex h-full flex-col bg-slate-950">
      {/* ── Header ───────────────────────────────────── */}
      <header className="border-b border-slate-800 bg-slate-900/50 p-6 md:p-8 shrink-0">
        <h1 className="text-2xl font-bold text-white tracking-tight mb-2">My Global Tickets</h1>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="mt-2 text-slate-400 max-w-sm">
              You don't have any tickets matching the current criteria across your projects.
            </p>
          </div>
        ) : (
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y divide-slate-800/50 text-slate-300">
                  {filteredTasks.map((task) => (
                    <tr
                      key={task._id}
                      onClick={() => navigate(`/projects/${task.projectId?._id}/board?taskId=${task._id}`)}
                    ...
                    >
                      ...
                      <td className="px-6 py-5">
                        <span className={`inline-flex items-center rounded-md border px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider ${getPriorityColor(task.priority)}`}>
                          {task.priority}
                        </span>
                      </td>
                      <td className="px-6 py-5">
