...
import { useGetProjectByIdQuery, useDeleteProjectMutation, useTogglePinProjectMutation } from "../features/projects/projectApi";
import { useSelector } from "react-redux";
import { selectCurrentUser } from "../features/auth/authSlice";
import { Settings as SettingsIcon, Users, Trash2, Globe, Shield, Pin } from "lucide-react";
...
const Settings = () => {
    ...
    const handlePin = async () => {
      try {
        ...
        // Toast is handled by UI response or we can add one here
      } catch (err) {
        console.error("Failed to pin project", err);
      }
    };

    if (isProjectLoading) return <div className="p-8 text-center text-slate-500">Loading project settings...</div>;
            ...
            {activeTab === "general" && (
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="mb-1 block text-sm font-medium text-slate-300">Project Name</label>
                            <input
                                type="text"
            ...
            {activeTab === "access" && (
                <div className="space-y-6">
                    <div>
                         ...
                         <p className="text-sm text-slate-400">Manage who can access this project.</p>
                    </div>
                    
                    <div className="rounded-lg border border-slate-700 bg-slate-800/30 p-4">
                        <p className="text-sm text-slate-300">
                            To manage team members, please visit the <button onClick={() => navigate(`/projects/${projectId}/team`)} className="text-indigo-400 hover:underline">Team Page</button>.
                        </p>
                    </div>

                    {/* Placeholder for role settings */}
                    <div className="space-y-4 opacity-50 pointer-events-none">
                         <h3 className="text-sm font-medium text-slate-300">Default Member Role</h3>
            ...
            {activeTab === "integrations" && (
                <div className="space-y-6">
                     <div>
                         ...
                         <p className="text-sm text-slate-400">Connect with third-party tools.</p>
                    </div>
                    
                    <div className="grid gap-4 md:grid-cols-2">
                        {['GitHub', 'Slack', 'Jira', 'Figma'].map((tool) => (
                             <div key={tool} className="flex items-center justify-between rounded-lg border border-slate-700 bg-slate-800/30 p-4">
            ...
            {activeTab === "danger" && (
                <div className="space-y-6">
                    <div>
                        ...
                        <p className="text-sm text-slate-400">Irreversible actions for this project.</p>
                    </div>
