...
const History = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "completed":
        return (
          <span className="bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 px-2 py-0.5 rounded-full text-xs font-medium uppercase tracking-wider">
            Done
          </span>
        );
      case "cancelled":
        return (
          <span className="bg-slate-500/10 text-slate-400 border border-slate-500/20 px-2 py-0.5 rounded-full text-xs font-medium uppercase tracking-wider">
            Cancelled
          </span>
        );
      case "rejected":
        return (
          <span className="bg-red-500/10 text-red-400 border border-red-500/20 px-2 py-0.5 rounded-full text-xs font-medium uppercase tracking-wider">
            Rejected
          </span>
        );
      default:
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        {task.completedAt ? (
                          ...
                          <div className="text-slate-300">
                            <span className="text-slate-500">Canc: </span>
                            {format(new Date(task.cancelledAt), "MMM d, yyyy")}
                          </div>
                        ) : task.rejectedAt ? (
                          <div className="text-slate-300">
                            <span className="text-slate-500">Rej: </span>
                      ...
                      <td className="px-6 py-4 max-w-[200px]">
                        <div className="truncate text-slate-400 text-xs">
                          {task.cancellationReason || task.rejectionReason || (
                            <span className="text-slate-600">-</span>
                          )}
                        </div>
                      </td>
