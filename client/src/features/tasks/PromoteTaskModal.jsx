...
const PromoteTaskModal = ({ isOpen, onClose, task, projectId }) => {
  ...
  const handleAddSubtask = (e) => {
    ...
    setSubtasks([
      ...subtasks,
      { title: newSubtask.trim(), isCompleted: false },
    ]);
    setNewSubtask("");
  };
        ...
        <div>
          ...
          <div className="space-y-2 mb-2">
            {subtasks.map((st, index) => (
              <div
                key={index}
                className="flex items-center gap-2 bg-slate-900/50 p-2 rounded border border-slate-800"
              >
                <div
                  className={`size-4 rounded-full border-2 ${st.isCompleted ? "bg-indigo-500 border-indigo-500" : "border-slate-600"}`}
                />
