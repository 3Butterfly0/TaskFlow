...
const AssignedTasksModal = ({ isOpen, onClose, member, projectId }) => {
  ...
  const memberTasks = useMemo(() => {
    ...
    return tasksData.data.filter((task) =>
      task.assignees?.some((a) => a === member._id || a._id === member._id),
    );
  }, [tasksData, member]);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Tasks for ${member?.username || "Member"}`}
    >
            ...
            {memberTasks.map((task) => (
              ...
              >
                <div className="flex items-start justify-between gap-3 relative">
                  ...
                  <div
                    className={`shrink-0 rounded bg-slate-800/80 px-2 py-1 text-[10px] font-bold uppercase tracking-wide border border-slate-700/50 ${
